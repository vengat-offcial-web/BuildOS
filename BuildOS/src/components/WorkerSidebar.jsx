...
export function WorkerSidebar() {
    ...
    return (
        <>
            {/* Mobile Header Bar */}
            <div className="md:hidden flex items-center justify-between p-4 bg-slate-900 border-b border-slate-800 z-30 sticky top-0">
                <div className="flex items-center gap-2.5">
                    <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-emerald-600 to-teal-500 flex items-center justify-center text-white text-lg shadow-md shadow-emerald-500/20">
                        <FaHelmetSafety />
                    </div>
                    <div>
                        <span className="text-lg font-extrabold bg-gradient-to-r from-emerald-400 to-teal-300 bg-clip-text text-transparent tracking-wide">BuildOS</span>
                        <span className="text-[10px] block text-slate-400 font-medium">Worker Portal</span>
                    </div>
                </div>
                <button
                    type="button"
                    ...
                    aria-label="Toggle Worker Menu"
                    className="p-2 rounded-xl bg-slate-800 text-emerald-400 hover:bg-slate-700 transition-all cursor-pointer"
                >
            ...
            {mobileOpen && (
                <div
                    ...
                    aria-hidden="true"
                    className="fixed inset-0 bg-black/60 backdrop-blur-xs z-35 md:hidden transition-opacity duration-300"
                />
            )}

            {/* Worker Sidebar Navigation Drawer */}
            <aside className={`
                fixed md:sticky top-0 left-0 z-40 w-72 md:w-64 h-screen p-5 flex flex-col shrink-0 transition-transform duration-300
                bg-slate-900/95 md:bg-slate-900/80 backdrop-blur-xl border-r border-slate-800/80
            ...
            `}>
                ...
                {/* Navigation Menu */}
                <nav aria-label="Worker Navigation" className="flex-1 space-y-1.5">
